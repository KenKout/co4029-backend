...
async def get_user_primary_organization_id(db: AsyncSession, user_id: UUID) -> UUID | None:
    ...
    intentionally excluded by the upstream surface — platform admins do
    not implicitly belong to one org and must hit endpoints that accept
    an explicit org param. Returns ``None`` for users with no membership
    AND no org-scoped role; the router then treats the catalog as empty.
    """
