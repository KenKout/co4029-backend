    ...
    *,
    ...
    stmt = select(InterviewSession).where(
        ...
        or_(
            ...
            InterviewSession.evaluation_claim_expires_at <= utcnow(),
        ),
        # A durable ACTIVE recovery job (dispatching/queued/running/retrying)
        # is the last re-drive still working; terminal phases stay visible.
        or_(
            current_phase.is_(None),
            current_phase.notin_(("dispatching", "queued", "running", "retrying")),
        ),
    )
    return list((await db.execute(stmt)).scalars().all())
