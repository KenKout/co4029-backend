...
_OCR_SYSTEM_PROMPT = (
    ...
    "interpret or add commentary.\n"
    "If the page contains a table, matrix or grid, transcribe it as a GitHub "
    "markdown table: one row per source row, the row-header in the first "
    "column, and every cell kept in its own column. Never flatten a grid into "
    "a running list — a cell read into the wrong row or column silently "
    "changes what the page asserts. If a cell is empty, emit an empty cell. "
    "Keep any cell numbering visible on the page.\n"
    "Return JSON of shape {\"text\": \"<content>\"}. "
