...
async def list_documents(
    ...
    *,
    ...
    The audience is LITERAL. ``role_codes=None`` (signed out) sees only the
    public set — policies with no audience. A signed-in reader additionally
    sees every policy naming one of their roles, and ONLY those: naming
    ``student`` means students, not everyone (the old universal-role reading
    was removed — a teacher picking through their index should not be shown
    documents that govern students specifically, and an admin choosing
    ``student`` in the picker means exactly students).

    An unrecognised code simply matches nothing rather than erroring: this
    is a courtesy filter over public documents, not a gate.
    """
