...
async def _seed_quiz(
    ...
    *,
    ...
    async with engine.begin() as conn:
        ...
        await conn.execute(
            text(
                "INSERT INTO quizzes (id, course_id, module_id, title, status, initial_ef, slug) VALUES (:id, :course, :module, :title, 'draft', :initial_ef, 'slug-' || uuid_generate_v4()::text);"
            ),
            {
                "id": quiz_id,
                "course": course_id,
...
async def test_first_review_correct_q5_calibrated(
    ...
    # Seed well below the EF ceiling: EF_MAX=2.5 (0069_sr_ef_ceiling) clamps
    # any positive delta from 2.5 to 2.5, which would make the dampening
    # assertion below pass vacuously. At 2.0 the growth is visible.
    student_id, question_id, attempt_id = await _seed_quiz(engine, initial_ef="2.00")
    async with session_factory() as session, session.begin():
        result = await record_card_review(
            session,
            student_id=student_id,
    ...
    # So delta = 0.1 * 0.6 * 0.5 = 0.03 → EF 2.0 → 2.03. (A free-recall format
    # would keep the full 0.1*0.6 = 0.06; see test_ef_update for the unit-level
    # guess-scale coverage.)
    expected_ef_after = 2.0 + 0.6 * 0.1 * 0.5
