...
async def test_course_outcomes_frozen_once_published(
    ...
    Runs on its OWN course, never the shared ``course_a``. Publishing is
    irreversible, so doing this to the seeded course left it published for
    every later LO test — and, because the harness does not reset the test
    database between runs, for every later RUN as well.
    """
    auth = {"Authorization": f"Bearer {admin_bearer}"}
    ...
    try:
        ...
        await _publish_ready(engine, course_id)
        pub = await client.post(f"/api/v1/teacher/courses/{course_id}/publish", headers=auth)
        ...
        revert = await client.patch(
            f"/api/v1/teacher/courses/{course_id}", json={"status": "draft"}, headers=auth
        )
        assert revert.status_code == 409, revert.text
        still = (
            await client.get(f"/api/v1/teacher/courses/{course_id}", headers=auth)
        ).json()
        assert still["status"] == "published"
    finally:
        # Hard-delete; the endpoint only soft-deletes, and a published course
        # left behind is exactly what this test used to leak.
